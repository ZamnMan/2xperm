-- -hats 123305754996947,127671621543010,80629770394501
...
_G.CH = {
    ...
    LeftLeg = {
        Name= "Accessory (Left Void Wall)",
        TextureId = "17269487469", 
        Orientation= CFrame.Angles(math.rad(0),math.rad(0),math.rad(90))
    },

    RightLeg = {
        Name= "Accessory (Right Void Wall)",
        TextureId = "17269487469", 
        Orientation= CFrame.Angles(math.rad(0),math.rad(0),math.rad(90))
    }
}
...
local function redo()
	...
	if not workspace:FindFirstChild("non") then
		char = game:GetObjects("rbxassetid://129709921260373")[1]
	else
		char = workspace.non
	end
	...
	connect = game:GetService("RunService").Heartbeat:Connect(function(set)
		...
		pcall(function()
			if nlia == false then 
				c["gooblet"].Handle.CFrame = char["Left Arm"].CFrame * CFrame.Angles(math.rad(0),math.rad(0),math.rad(0))  +antisleep
			else 
				c["gooblet"].Handle.CFrame = char["Left Arm"].CFrame * CFrame.Angles(math.rad(0),math.rad(0),math.rad(0))  +antisleep
			end
		end)
		pcall(function()
			if nria == false then 
				c["fooblet"].Handle.CFrame = char["Right Arm"].CFrame * CFrame.Angles(math.rad(0),math.rad(0),math.rad(0))+antisleep
			else 
				c["fooblet"].Handle.CFrame = char["Right Arm"].CFrame * CFrame.Angles(math.rad(0),math.rad(0),math.rad(0))+antisleep
			end
		end)
		pcall(function()
			c["Accessory (rightleg)"].Handle.CFrame = char["Right Leg"].CFrame  * CFrame.Angles(math.rad(0),math.rad(0),math.rad(0))   +antisleep
		end)
		pcall(function()
			c["Accessory (LLeg)"].Handle.CFrame = char["Left Leg"].CFrame  * CFrame.Angles(math.rad(0),math.rad(0),math.rad(0))   +antisleep
		end)

		pcall(function()
			for _,v in pairs(c:GetChildren()) do
				if v:IsA("Accessory") and v.Name ~= "gooblet" and v.Name ~= "fooblet" and v.Name ~= "Accessory (HeadAccessory)"   and v.Name ~= "BlueRobotArm" and v.Name ~= "GreyRobotArm"   and v.Name ~= "Tor" and v.Name ~= "PurpleRobotArm" and v.Name ~= "RobotArmTest"  and v.Name ~= "LARM" and v.Name ~= "RARM" and v.Name ~= "Accessory (rightleg)" and v.Name ~= "Accessory (LLeg)" then  
...
pcall(function()
	...
	if _G.ToolFling == false then
		velocity = game:GetService("RunService").Heartbeat:Connect(function()
			for i,v in pairs(c:GetChildren()) do
				if v:IsA("Accessory") or v:IsA("Tool") then
					pcall(function()
						v.Handle.AssemblyLinearVelocity = Vector3.new(char.Torso.AssemblyLinearVelocity.X*15 -10*math.clamp(math.sin(os.clock()*10000000),0.5,1),   28.7849+math.sin(os.clock()*10000), char.Torso.AssemblyLinearVelocity.Z*15 +6.12463345 ) 
						v.Handle.AssemblyAngularVelocity = Vector3.new(0, sin(clock()*11000000000), 0)
						v.Handle.CanCollide = false
					end)
				end
			end
		...
		velocity = game:GetService("RunService").Heartbeat:Connect(function()
			for i,v in pairs(c:GetChildren()) do
				if v:IsA("Accessory")  then
					v.Handle.AssemblyLinearVelocity = Vector3.new(char.Torso.AssemblyLinearVelocity.X*15,28.794, char.Torso.AssemblyLinearVelocity.Z*15) 
		...
		pcall(function()
			...
			pcall(function()
            	...
            	pcall(function()
                        ...
                        c["Accessory (HeadAccessory)"].Handle.CFrame = char["Head"].CFrame  * CFrame.Angles(0,math.rad(180),0)  +antisleep+ char["Head"]:FindFirstChildOfClass("SpecialMesh").Offset 
                    else
                     c["Accessory (HeadAccessory)"].Handle.CFrame = char["Head"].CFrame  * CFrame.Angles(0,math.rad(180),0) +antisleep
                    end
			end)
			pcall(function()
				for _,v in pairs(c:GetChildren()) do
			...
			if plr.Character.Name ~= "non" then
				if plr.Character.Name ~= "non" then
					if _G.TeleportCharacter == true then
                        if _G.HatCollide == false then
						plr.Character:WaitForChild("HumanoidRootPart").CFrame =CFrame.new( workspace.non.HumanoidRootPart.CFrame.X  , workspace.FallenPartsDestroyHeight + 450, workspace.non.HumanoidRootPart.CFrame.Z  ) 
                        else 
	plr.Character:WaitForChild("HumanoidRootPart").CFrame =CFrame.new( workspace.non.HumanoidRootPart.CFrame.X  , workspace.FallenPartsDestroyHeight + .25, workspace.non.HumanoidRootPart.CFrame.Z  ) 
